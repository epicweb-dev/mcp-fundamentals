...
<EpicVideo url="https://www.epicai.pro/workshops/mcp-fundamentals-mtezq/intro~xrkgx" />
...
Resources and resource templates can also include optional metadata such as
`title`, `description`, and `icons` to help clients present them nicely. Keep
the URI as the stable identifier and use display metadata for the user
experience.
...
<callout-warning>
	...
	server. The rest of the user experience is controlled by the creator of the
	client.
</callout-warning>
...
<callout-info>
	The `ui://` URI scheme is reserved for MCP Apps UI templates. This workshop
	uses data-oriented examples like `epicme://...` and avoids `ui://` because UI
	resources are covered separately.
</callout-info>
...
<callout-warning>
	...
	we should just use resources for. Client support for resources varies by host
	and continues to improve, so we'll keep those tools around for compatibility
	while still exposing resources where they make sense.
</callout-warning>
