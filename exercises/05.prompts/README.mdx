...
<EpicVideo url="https://www.epicai.pro/workshops/mcp-fundamentals-mtezq/intro~cndmc" />
...
Prompt argument schemas are also JSON Schema-backed. In this workshop we use
Zod through the TypeScript SDK, which generates schema that matches the current
MCP defaults for us.
...
<callout-info>
	Some MCP metadata types now support richer display metadata such as icons. The
	current high-level TypeScript SDK prompt registration helper exposes `title`,
	`description`, and `argsSchema`, so we'll keep prompts focused on those fields
	here.
</callout-info>
...
Prompt list change notifications are useful for dynamic servers, but they are an
advanced topic covered outside this fundamentals workshop.
