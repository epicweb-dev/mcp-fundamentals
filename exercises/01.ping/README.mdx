...
<EpicVideo url="https://www.epicai.pro/workshops/mcp-fundamentals-mtezq/intro~xh5mj" />
...
<callout-info>
	HTTP transports continue to evolve. For example, modern Streamable HTTP
	clients include the `MCP-Protocol-Version` header on subsequent requests, and
	upcoming transport proposals are exploring more stateless HTTP patterns and
	routing headers like `Mcp-Method` and `Mcp-Name`. This workshop intentionally
	avoids those transport details so we can focus on the server primitives that
	stay the same across transports.
</callout-info>
